...
class WorkWithArrays {
    ...
    static int[] evenElements(final int[] array) {
        final int arrayLenght= array.length % 2 == 0 ? array.length/2 : array.length/2+1;
        final int[] resultArray= new int[arrayLenght];
    ...
    static int mostRecurringElement(final int[] array) {
        int mostRecurring=0;
        int maxOccorrencies=0;
        final int[] copyArray= Arrays.copyOf(array, array.length);
        Arrays.sort(copyArray);

        for(int i=0; i<copyArray.length; i++){
            final int currentElement=copyArray[i];
            int occurrencies=0;
            for(i++; i<copyArray.length;i++){
                if (copyArray[i]==currentElement) {
                    occurrencies++;
                    if (occurrencies> maxOccorrencies) {
                        maxOccorrencies=occurrencies;
                        mostRecurring=currentElement;
                    }
                }
            }
        }
    ...
    static int[] revertUpTo(final int[] array, final int element) {
        ...
        for(; i<=array.length && array[i]!= element; i++){
            int k=0;
            for(;k<=i; k++){
                returnArray[k]=array[i-1];
            }
            for(; k<array.length;i++){
